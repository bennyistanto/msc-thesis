...
%          chapters. Ordered to follow the thesis story: reference aid
%          first, then data, then implementation, then extended results.
% Sections: A Glossary, B Data Catalogue, C Configuration Template,
%           D Code and Data Availability, E Reproducibility Checklist,
%           F Per-Station Verification, G Spatial Atlas,
%           H Supplementary Temporal-Alignment Diagnostics.
%           Cross-references go through \ref{app:...} so the letters renumber
%           automatically; never hardcode them.
...
% ----- APPENDICES divider: a page of its own (opens on a recto) -----
% PPKI: the word APPENDICES sits alone on an odd page, centred both
% horizontally and vertically, 14pt bold. The appendix content (A. DATA
% CATALOGUE onward) starts on the FOLLOWING page, so the \clearpage at the end
% of this block is load-bearing, not cosmetic.
\cleardoublepage
\thispagestyle{fancy}
\phantomsection
...
\end{center}
\vspace*{\fill}
% Nest the appendix chapters (A-H) UNDER the APPENDICES bookmark: demote their
% hyperref outline levels by one (chapter->1, section->2, subsection->3) so the
% PDF bookmark panel shows them inside APPENDICES. Reset before the biography in
% main_thesis.tex so BIOGRAPHY returns to the top level. Printed TOC/LoA layout
% is unaffected (this only changes the bookmark nesting).
\makeatletter
\renewcommand{\toclevel@chapter}{1}%
\renewcommand{\toclevel@section}{2}%
\renewcommand{\toclevel@subsection}{3}%
\makeatother
% End the divider page; the first appendix opens on the next one.
\clearpage
...
% same page (a new page is taken only when the content runs over). Applied
% here, after the main chapters, so only the appendices are affected;
% main_thesis.tex restores the body chapter style afterwards.
\makeatletter
...
Satellite data were obtained from the
...
Together these drop 8 of the 180 archived stations, leaving 172 for
the dekadal correction metrics, the multi-threshold verification,
and the dry-day summaries.

Analyses with different coverage requirements retain different
subsets of the same archive, and the counts should not be
interchanged:
...
Analysis & Stations \\
\midrule
Archived BMKG network & 180 \\
Within the study window; per-station Taylor; half-hourly window diagnostic & 178 \\
Day-label scan & 177 \\
Dekadal correction metrics; multi-threshold; dry-day & 172 \\
Valid per-dekad correlation (monthly Taylor) & 171 \\
\bottomrule
\end{tabular}
\end{center}

\noindent
The $0.1^{\circ}$ grid resolves the 180 stations onto 169 distinct
pixels, since some stations share a pixel; at the CPC-UNI
$0.5^{\circ}$ resolution they fall in 134 cells.
...
none of which was formally optimised, are all set here, which is
what makes the sensitivity sweep of
...
run without any external download. Because the repository continues
to develop, the citable record is an archived snapshot of it on
Zenodo: \url{https://doi.org/10.5281/zenodo.21672535}. That DOI
names the exact release, v2026.07, this thesis was written against.
...
validation, and figures, together with the $36$ per-dekad CNN
models, approximately $40$~GB) are held in a separate data deposit,
along with the derived arrays behind the temporal-alignment
analysis of Section~\ref{sec:methods-window}:
\url{https://doi.org/10.5281/zenodo.21640696}. That DOI names the
exact deposit version this thesis was written against.
To reproduce the
Indonesia results, a reader clones the repository, downloads the
Zenodo bundle, points \texttt{config.yml} at the extracted
directories, and runs the notebooks. The companion paper is published in
\emph{Remote Sensing} \citep{ref:istanto2026mdpi}
(\url{https://doi.org/10.3390/rs18142298}).
...
deposit (full Indonesia):
\url{https://doi.org/10.5281/zenodo.21640696}.

\medskip\noindent\textbf{Window diagnostics.}
The temporal-alignment results are the one part of this work that
the notebook pipeline does not produce. They are computed from the
half-hourly IMERG-L archive rather than the daily inputs, by way of
a per-station half-hourly cache and a set of derived offset arrays
holding additive sufficient statistics keyed by the offset $h$.
Reproducing these figures therefore requires either the half-hourly
archive or those derived arrays; the arrays, the scripts that
generate and consume them, and the dependencies between them are
documented with the data deposit.
...
all $36$ dekadal windows, the notebook sequence
nb02 to nb06 completes in $72.1$ minutes
(Table~\ref{tab:colab-runtime}). The preparatory notebooks nb00
(area-of-interest definition) and nb01 (data download) are
excluded: both scale with the extent of the domain requested and
with network throughput, so neither produces a runtime another
user could reproduce or meaningfully compare. The full Indonesian
domain scales up with pixel count and is correspondingly longer.
...
The re-run reported in Section~\ref{sec:results-reproducibility}
uses the Bali subdomain shown below, chosen because it is small
enough to complete on a free cloud instance while still
...
the daily station records over the whole 2001 to 2021 record at
the archived UTC date label, without the day-label matching of
Section~\ref{sec:results-intercomparison}. The table illustrates
the correlation pattern at the level of individual stations: the
Pearson correlation stays close between the raw IMERG-L and the
corrected LSEQM+DL series, while the standard deviation of the
corrected series moves toward the observed standard deviation.

Two comparisons should not be drawn from these four rows. They are
individual stations, not a network summary: the per-station median
over the 178 stations of this diagnostic is $0.286$ for IMERG-L and
$0.280$ for LSEQM+DL, and the coincidence between that median and
the Kahang-Kahang entry is arithmetic accident. They are also
measured at the archived label, so they are not comparable with the
$0.57$ reported at the matched label in
Chapter~\ref{chap:results}. The two stations that decline,
Kahang-Kahang and Jembrana, do so by more than the sampling noise
of a full-record sample and are discussed with the
representativeness argument below. The full per-station tables are
in the Zenodo release.
...
\newcommand{\atlasGoneWidth}{0.95}   % G.1 difference maps
\newcommand{\atlasGtwoWidth}{0.95}   % G.2 three-day x three-product
\newcommand{\atlasGthreeWidth}{0.95} % G.3 pillar metrics
...
The final map isolates the Stage-4 contribution to spatial
quality. It is reported here rather than in
Section~\ref{sec:results-cqi} because the change it shows is
small, and the values that matter are stated numerically there.
...
This diagnostic runs to $2025$ because both the satellite archive
and CPC-UNI extend that far. The station-based diagnostics of
Chapter~\ref{chap:results} end in $2021$, where the BMKG archive
ends, so their GPM era is $2015$ to $2021$. The two windows give
similar results here, a pooled whole-domain correlation of $0.325$
to $2025$ against $0.334$ to $2021$, but the window should be
stated whenever either value is quoted.

The optimum is the same in the timing-imprecise TRMM-input era
(Figure~\ref{fig:grid-trmm}) as in the GPM era
(Figure~\ref{fig:grid-gpm}), which is consistent with the offset
being a
...
The two figures below support the stability claims made in
Section~\ref{sec:results-window}. They are reported here rather
than in the body because each is a robustness check on a result
already stated numerically there: the peak offset does not move
with season, and it does not move with longitude.
...
% The deep-learning refinement footprint is presented in
% Section~\ref{sec:results-stages}, Figure~\ref{fig:cnn-footprint}.
