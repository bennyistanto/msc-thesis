...
Maluku and Papua \citep{ref:ramadhan2022, ref:rahmawati2021,
ref:mashuri2025, ref:chen2008}.
...
limiting factor \citep{ref:ramadhan2022ts, ref:ramadhan2023,
ref:tang2020}. Part of
...
period to the gauge convention \citep{ref:xie2022, ref:salio2015,
ref:tan2019, ref:li2018}, but to our knowledge it has not been
...
The study pursues five objectives:

\begin{enumerate}
    \item To present the LSEQM+DL framework as an integrated,
    configuration-driven workflow and apply it to IMERG-L over
    Indonesia for the 2001 to 2025 record.
    \item To characterise, stage by stage against 172 independent
    Indonesian Meteorological, Climatological, and Geophysical
    Agency (BMKG) stations, which bias dimension each of the four
    stages corrects and by how much.
    \item To establish the framework's reach and its limit along
    the day-by-day timing dimension: to test, empirically and
    theoretically, the bound on the Pearson correlation between
    corrected daily satellite and gauge values under marginal
    correction, locating the observed value within the empirical
    band reported for daily IMERG over the tropics; and to
    decompose that ceiling into a recoverable calendar-window
    component and a residual retrieval-skill component by
    re-aggregating the half-hourly satellite source to a window
    matched to the gauge reporting convention, characterising the
    optimal window across season, space, and satellite era.
...
Climate Prediction Center (CPC) Unified Gauge-Based Analysis of Global Daily
Precipitation (CPC-UNI) gauge analysis and validated against 172 BMKG stations.
...
the unchanged correlation alongside the corrected distribution, and
...
The decomposition of that timing bound. The daily correlation
ceiling is shown to be dominated, in the GPM era, by the
...
\begin{description}[noitemsep, topsep=2pt]
    ...
    \item[H3.] In the GPM era, the daily Pearson correlation is
    suppressed primarily by the calendar-window mismatch between the
    UTC-day satellite archive and the local-day gauge convention, so
    that re-aggregating the satellite to a gauge-matched window
    recovers a large part of the correlation; the recovery is absent
    in the earlier era, in which the retrieval does not resolve the
    diurnal cycle.
\end{description}
