...
the daily IMERG-L archive to a window matched to the local-day
gauge convention shared by CPC-UNI and BMKG would lift the
headline Pearson~$r$ from approximately $0.34$ to approximately
$0.62$ with no change to any other pipeline output, so a
locally-aligned re-aggregation should be considered before the
product is rejected as unskilful.
...
calendar-window component that accounts for approximately
two-thirds of the gap and is addressable by re-aggregating the
daily IMERG-L archive over a 24-hour window matched to the
local-day gauge convention shared by the CPC-UNI calibration
target and the BMKG validation network; this is a single-stage
