...
This thesis asked what a marginal-correction bias correction
framework can and cannot do for a daily satellite precipitation field, and it
...
Indonesia for 2001 to 2025 and validated against 172 independent
...
ceiling: the sub-daily diagnostic in
Section~\ref{sec:discussion-subdaily} shows that re-aggregating
the daily IMERG-L archive to the local-observation window would
lift the Pearson~$r$ against the BMKG gauges from approximately
$0.20$ to approximately $0.57$ in the GPM era, with no change to
any other pipeline output, so a locally-aligned re-aggregation
should be considered before the product is rejected as unskilful.

\medskip\noindent\textbf{For researchers extending the method.}
Section~\ref{sec:discussion-subdaily} decomposes the apparent
Pearson~$r$ ceiling into two components. The first is a
calendar-window label offset that suppresses the correlation
against the local-day BMKG gauges and is addressable by
re-aggregating the daily IMERG-L archive to the local-observation
window (the CPC-UNI calibration correlation carries no such
offset); this is a single-stage
input-side change that does not modify the LSEQM+DL pipeline and
is the first action item for any extension. The second is a
residual retrieval-skill component that requires methods outside
the marginal correction family: multivariate quantile mapping,
sub-daily disaggregation against the native half-hourly product,
or stochastic disaggregation with ensemble verification. The
three sensitivity parameters explored one-at-a-time in
...
allows but does not by itself establish. A fully-convolutional
variant of the Stage~4 network, prototyped on the Bali subdomain,
matched the dense model's station-validation skill while using
about $900$ times fewer parameters at the full Indonesia
resolution; replacing the dense reconstruction head with it
across Indonesia is left to future work.
