...
the daily IMERG-L archive to a morning-observation window matched
to the BMKG gauge convention would lift the headline Pearson~$r$
from approximately $0.34$ to approximately $0.62$ with no change
to any other pipeline output, so a locally-aligned re-aggregation
should be considered before the product is rejected as
unskilful.
