...
%           3.7 Window-Offset Timing Diagnostic,
%           3.8 Reproducible Implementation Design, 3.9 Chapter summary.
...
metric catalogue (Section~\ref{sec:methods-metrics}), the
window-offset timing diagnostic (Section~\ref{sec:methods-window}),
and the reproducible implementation that supports independent
verification (Section~\ref{sec:methods-reproducibility}). The algorithmic details
summarised here are extended from a parallel paper
...
framework that the parallel paper reports in compressed form.

Figure~\ref{fig:framework} maps the study from its background and
objectives to the findings and contribution, with the dash-dot boundary
marking the research scope developed in the sections below.

\begin{figure}[H]
\centering
\includegraphics[width=\figThreeFrameworkWidth\textwidth]{fig_thesis_03_framework.png}
\caption[Research framework of the study]{Research framework of the study: from background and objectives, through the approach and its verification, diagnostic, and reproducibility groups, to the findings and contribution, with the dash-dot boundary marking the research scope.\label{fig:framework}}
\end{figure}

The framework begins from a three-part gap: daily IMERG-L carries
systematic bias over the Maritime Continent, aggregate skill scores
conceal which bias dimension fails, and published corrections are
seldom reproducible. Within the research scope the study addresses this
through the data, the four-stage correction, and three working groups,
each tagged with the objective it serves. Verification (O1) scores all
three corrected products on 31 metrics and a Composite Quality Index.
The diagnostic group (O2) isolates the day-by-day correlation ceiling
and the calendar-window offset that suppresses it, and the
reproducibility group (O3) releases the pipeline as a
configuration-driven, cloud-runnable implementation. The findings are a
corrected daily product, a stage-resolved account of what each stage
fixes, and a timing ceiling with a recoverable calendar-window
component; the contribution is an open, transferable framework whose
verification states plainly what it does and does not correct.
...
Natural Sciences, Bogor Agricultural University (IPB), from
March 2024 to February 2026.

Data processing and model development used the Python programming
language \citep{ref:vanrossum2009python} and its scientific
computing stack, run on a local workstation and re-run on Google
Colab for cloud-based reproducibility checks. The pinned package
versions, the hardware, and the remaining software details are
given in Section~\ref{sec:methods-reproducibility} and
Appendix~\ref{app:code}.

All source code, notebooks, configuration, and the input and output
data referred to throughout this chapter, including the \texttt{src/}
modules cited by name in the sections that follow, are openly
released; the GitHub repository and the archived Zenodo dataset, with
their citable identifiers, are given in the Code and Data Availability
statement (Appendix~\ref{app:code}).
...
Measurement (IMERG) Version 7 (V07)~\citep{ref:huffman2020} is
the product to be corrected. The CPC Unified Gauge-Based Analysis of Global Daily
...
latitude from $6^{\circ}$N to $11^{\circ}$S
(Figure~\ref{fig:coverage}). It occupies the
...
contrasts with the comparatively drier eastern provinces.

The convective diurnal cycle matters for daily aggregation. It
peaks at local 14:00 to 17:00 (UTC+7 to
UTC+9), which places the peak in the UTC 07:00 to 10:00 band for
the western archipelago and earlier in UTC terms for the east. The
peak therefore sits well inside a single UTC day, and the calendar
mismatch that follows is one of date labelling rather than of a
convective afternoon split across two days
(Section~\ref{sec:methods-window}). The gauge
...
% Fig 3.1: study-area data coverage, dedicated page, image rotated 90 deg to
% occupy the page height (same approach as Figs 3.2/3.3). Anchored here in
% Section 3.2.1 so the [p] float lands before the dataset subsections and is
% not on the same spread as the Fig 3.2 workflow page.
\begin{figure}[p]
\centering
\rotatebox{90}{%
  \includegraphics[width=\figThreeOneWidth\textheight]{fig_thesis_03_coverage.png}%
}
\caption[Study-area data coverage over Indonesia]{The IMERG-L $0.1^{\circ}$ land footprint ($19{,}395$ pixels), the CPC-UNI $0.5^{\circ}$ native calibration grid, and the $180$ archived BMKG stations by region\label{fig:coverage}}
\end{figure}

\subsection{IMERG Late Run}

IMERG V07 provides a quasi-global precipitation product at
$0.1^{\circ}$ (approximately 11~km) and 30-minute resolution; its
retrieval architecture is described in
Section~\ref{sec:lit-imerg}, along with its three latency
variants. The framework operates on the Late
...
Indonesian bounding box before processing, yielding the
$0.1^{\circ}$ land footprint of $19{,}395$ pixels mapped in
Figure~\ref{fig:coverage}. The same Late Run is
...
$0.5^{\circ}$ (approximately 55~km) global land grid, the coarse
calibration grid drawn against the finer IMERG footprint in
Figure~\ref{fig:coverage}. The product
...
contributing network is concentrated in Java and Sumatra.

The
contributing observations follow the World Meteorological
Organization (WMO) local-day morning-observation convention of
their reporting stations, but the archive does not carry that
convention into its date label. Each daily total is dated by a
per-cell end-of-day field, and for every Indonesian cell that
field is $24$~hours, so the total closes at $00{:}00$~UTC and is
assigned the UTC calendar date \citep{ref:cpcuni_readme}. The
CPC-UNI daily total therefore shares the accumulation window of
the UTC-dated IMERG-L archive, while the BMKG archive keeps the
local morning-read date one calendar day later. This distinction
between the observation practice and the date label is what
separates the two gauge references, and it is central to the
Pearson~$r$ ceiling discussed in
...
BMKG operates a national synoptic station network~\citep{ref:bmkg2006sk32}
whose daily precipitation archive serves as the independent
validation reference. Independence here has a specific meaning. Some BMKG
...
Appendix~\ref{app:data} catalogues the datasets, records the
station funnel in full, and documents the sentinel handling
summarised here. The 8 dropped stations
...
late in the 2001 to 2021 window.

The retained network spans the
seven major regions of the archipelago: Sumatra, Kalimantan,
Sulawesi, Java, Bali and Nusa Tenggara, Maluku, and Papua. The
full archived network is shown in Figure~\ref{fig:coverage},
together with the IMERG-L and CPC-UNI data grids. It illustrates
the gauge-density gradient referred to
...
\subsection{Suitability of the Calibration Reference}\label{sec:methods-reference-quality}

A correction can be no better than the reference it is fitted to,
so the choice of CPC-UNI and its limits over Indonesia are set out
here rather than left implicit.

The reference had to satisfy four requirements simultaneously: a
daily time step, matching the correction unit; gridded coverage of
the whole archipelago, since a station product cannot calibrate a
grid; continuous availability across the 2001 to 2025 study period;
and a latency short enough that the same reference could be used in
the operational setting the framework targets. A fifth requirement
follows from what the correction is for: the reference must be a
gauge-only analysis. Calibrating a satellite product against a
satellite-gauge blend would correct the product partly toward
satellite information it already contains, and the resulting skill
could not be attributed to the gauges.

That fifth requirement is what eliminates the alternatives in
Table~\ref{tab:gridded-products}, and it does so on the table's own
gauge-integration column. CHIRPS and MSWEP are satellite-gauge
ensembles and PERSIANN-CDR is a satellite product adjusted to a
monthly gauge analysis, so none is gauge-only and all three are
ruled out as calibration targets however attractive their
resolution or latency; ERA5-Land ingests no
gauges directly; and the IMERG Final Run belongs to the product
family being corrected. One gauge-only alternative sits outside
that table: the APHRODITE analysis for Asia
(Section~\ref{sec:lit-gauge-quality}) is denser than CPC-UNI but is
a regional research release rather than an operational product, so
it fails the continuity and latency requirements even though it
passes the gauge-only one. CPC-UNI is the analysis that meets all
five.

The three properties that govern the quality of any gauge-based
analysis are reviewed in Section~\ref{sec:lit-gauge-quality}. Over
Indonesia they act together rather than separately.
Indonesia is served by the routine GTS exchange rather than by any
of the dense national collections CPC ingests separately, so the
Indonesian contribution to the calibration reference is a fraction
of the BMKG network and varies from day to day with reporting
completeness. That contribution is drawn from a source archive that
is itself gappy: of the $1{,}380{,}600$ station-days spanned by the
$180$-station record described above, $36.2\%$ are missing, the
median station is missing $24.0\%$ of its days, $51$ of the $180$
stations are missing more than half, and completeness varies by
year from $22.7\%$ missing in 2011 to $50.9\%$ in 2015.

These counts treat a station-day as missing if it is either absent
or carries the $8888$ sentinel, and are reproduced from the archive
by \texttt{scripts/verify\_bmkg\_completeness.py} in the released
repository (Appendix~\ref{app:code}). The
reference is therefore best constrained over Java and southern
Sumatra and progressively less so eastward, in the same pattern as
the network that feeds it.

Three consequences run through the rest of the study. The
correction target is reliable where the contributing network is
dense and progressively less so where it thins, which is the
condition the station-density confidence mask of
Section~\ref{sec:methods-blending} responds to, following the
product documentation's own direction to use the gauge-count field
rather than treating every cell as equally well constrained.
Validation is performed against point observations extracted at
station pixels rather than against the gridded reference, which
avoids inheriting the interpolation step whose limits are described
here. And the distribution-alignment and tail stages of the
pipeline act on a reference whose extremes are documented to be
compressed by both interpolation and the $0.5^{\circ}$ grid, which
bounds how far the corrected extremes can be trusted and is
returned to in Section~\ref{sec:discussion-limitations}.
...
of a per-pixel statistical correction.

The four stages execute
...
% Flush the full-page pipeline float (Fig 3.3) before the bridge paragraph so
% the paragraph renders between Fig 3.3 and Fig 3.4, not ahead of both.
\clearpage

Where Figure~\ref{fig:pipeline} gives the conceptual view,
Figure~\ref{fig:execution} gives the execution view of the same
pipeline as a flowchart in standard symbols. It shows the dependency of
each stage on the previous stage's output, the dedicated output that
each stage writes so the stages can be verified independently, the
decision that governs where the CNN contributes, and the verification
step that reads all three corrected products and writes the per-method
metrics.
The corrected products and the per-method gridded metrics against
CPC-UNI are stored as NetCDF written per dekad; the independent
comparison against the BMKG stations is a per-station validation
reported separately.

\begin{figure}[H]
\centering
\includegraphics[width=\figThreeExecWidth\textwidth]{fig_thesis_03_execution.png}
\caption[Execution flowchart of the LSEQM+DL pipeline]{Execution flowchart of the LSEQM+DL pipeline: stage dependency, per-stage outputs, the two-condition decision that activates the CNN, and verification of all three products against CPC-UNI and BMKG.\label{fig:execution}}
\end{figure}

Three things the figure leaves implicit are worth stating. The flow
shown executes once per dekadal window and is repeated across all
$36$ windows, so the pipeline produces $36$ independent sets of
fitted parameters. The Stage-4 network is trained on its own window
before it is applied, giving $36$ saved models rather than one.
And CPC-UNI enters at every fitting stage, not only at Stage~1: it
supplies the Gamma parameters in Stage~2, the tail parameters in
Stage~3, and the training target in Stage~4.
...
\citet{ref:wood2004}. LS corrects the dekadal mean by construction;
...
\label{eq:eqm}
\end{equation}
The two Gamma parameters describe the shape and the scale of the
bulk distribution, where most of the sample mass concentrates.
...
$0.5^{\circ}$ field would impose.

Figure~\ref{fig:bcsd} illustrates
...
The Gamma bulk fit of Stage~2 cannot represent the heavy upper
tail that real tropical convective rainfall exhibits: forced
through the light-to-moderate bulk, its fitted tail runs
systematically too short. The regime this stage is included to
preserve is quantified in Section~\ref{sec:results-threshold}: the
BMKG wet-day distribution is strongly right-skewed, and the raw
satellite input truncates the $99$th wet-day percentile to
$73~\mathrm{mm/day}$ against $106$ at the gauges. Stage~3
grafts a GPD onto the upper
tail, fitted explicitly to exceedances above a high threshold.
Following the peaks-over-threshold rationale of
Section~\ref{sec:lit-gpd}, the tail is modelled by a GPD with
shape and scale parameters $(\xi, \sigma)$ \citep{ref:coles2001},
...
\begin{enumerate}
    ...
    small per-pixel exceedance set (see below).
...
reference tail~\citep{ref:gudmundsson2012}. For a satellite value $P_{\text{LS}}$ above the
...
\label{eq:gpd-map}
\end{equation}
The reference GPD is fitted to the exceedances
$x - u_{\text{ref}}$ with its location pinned to zero, so
$G_{\text{ref}}^{-1}(p_c)$ returns an exceedance and
$u_{\text{ref}}$ is added back.
...
fit: the conventional range is the 90th to 99th wet-day
percentile (Section~\ref{sec:lit-gpd}). The present study made an empirical
...
fitting~\citep{ref:pickands1975}. Whether this holds depends on the local wet-day
...
trivial mapping $y = x$, leaving the Gamma EQM value in
...
pixel-dekads in which the adjustment is active rather than
fallback was not mapped; such a diagnostic would be a useful
supplementary artefact for future work.
...
climatological heterogeneity of the domain. There is no
smoothing across the join: the two curves are constrained to
match at the threshold percentile, so the discontinuity is small
in practice but is not exactly
...
The final Dense layer reconstructs the full grid and dominates the
parameter count, so the network is large relative to the per-dekad
training sample. The risk of overfitting is contained at inference
rather than by the architecture: the refinement is applied only to
extreme pixels, its contribution is capped through the blend
weight, and it is disabled where station density is low, as
discussed below. A fully-convolutional reconstruction head,
prototyped on the Bali subdomain, matches the dense model's
station-validation skill while cutting the Stage-4 parameter count
by roughly $900$ times at the full Indonesia resolution. The dense
head is retained here so that the released models reproduce the
numbers reported in this thesis; the fully-convolutional variant is
available through the \texttt{architecture} option as the
recommended extension for full-resolution deployment.
...
The split is implemented as a random
...
when the blend weight is set to $\alpha = 1$. The blend weight
...
spatial diagnostic of per-dekad CNN validation loss would
surface seasons in which the CNN contribution is weaker because
the training extreme-event sample is thinner, typically
dry-season dekads. It is not produced here and would be a useful
supplementary artefact for future work.
...
% [p]: at 555 pt the panel grid cannot share a page with the 24 lines that
% precede it, so [H] left 339 pt blank on the preceding page. Floating it to a
% dedicated page lets Section 3.4 fill that space instead.
\begin{figure}[p]
\centering
...
The CNN refinement in Stage~4 is trained against CPC-UNI~\citep{ref:chen2008},
and the quality of CPC-UNI itself depends on the local density of
...
the nominal value $\alpha = 0.70$ by the confidence field $C$:
\begin{equation}
...
The $C = 1$ case is a limit rather than an operating point. Over
Indonesia the smoothed confidence field reaches a maximum of
$0.709$, so the nominal $30\%$ is never realised anywhere in the
domain: the largest effective CNN contribution is about a fifth,
over Java, and the mean effective contribution across land pixels
is about $2\%$. The nominal weight is therefore a ceiling on the
refinement, not a description of it.
...
\citep{ref:hoskingwallis1997, ref:wilks2020}. Second, it produces
...
The metric catalogue itself is described in
...
and \textit{event detection} \citep{ref:ebert2007, ref:wilks2020}.
...
gauge first and second moments~\citep{ref:wilks2020}. The relative bias (RB) compares the
sample means:
\begin{equation}
...
\citep{ref:wilks2020} compares the empirical cumulative
...
addition, multi-threshold verification reports the categorical
skill of the corrected series at seven daily thresholds spanning
the operational range.

The thresholds up to 50~mm/day (1, 5, 10,
20, and 50) follow the multi-threshold verification approach of
the World Meteorological Organization (WMO) standard TD-1485
\citep{ref:wmo1485}. The 100 and 150~mm/day thresholds extend the range to
heavy and extreme classes relevant to the Indonesian wet-season
regime, consistent with the WMO principle that
extreme-precipitation thresholds are set regionally
\citep{ref:wmo1310}. At each threshold, hits $H$, false alarms
...
categorical metrics~\citep{ref:ebert2007, ref:wilks2020} aggregated across all wet days, that is,
...
combination $0.6\,\text{POD} + 0.4\,(1 - \text{FAR})$;
$S_{\text{RB}}$, $S_{\text{RMSE}}$, $S_{\text{NSE}}$,
$S_{\text{CSI}}$, and $S_{\text{spell}}$ are the normalised
relative-bias, RMSE, NSE, CSI, and wet-spell scores, respectively.
The three
...
\label{eq:cqi}
\end{equation}
The CQI construction follows the parallel paper
...
For qualitative interpretation the continuous CQI is binned into
four quality tiers using the boundaries set in \texttt{config.yml}:
Poor ($<0.40$), Fair ($0.40$ to $0.60$), Good ($0.60$ to $0.80$),
and Excellent ($\geq 0.80$). These tiers label the spatial maps in
Section~\ref{sec:results-cqi}; the underlying CQI is retained and
mapped continuously.
...
correlation set by the day-by-day pairing of the raw IMERG
input, which the correction does not change. The empirical verification of this expectation is the
...
the Fisher $z$-transform~\citep{ref:wilks2020} standard error
...
% ------------------------------------------------------------
\section{Window-Offset Timing Diagnostic}\label{sec:methods-window}
% ------------------------------------------------------------

The Pearson diagnostic of Equation~\ref{eq:pearson} is governed by
...
The daily IMERG-L archive accumulates over the UTC day, which is
also the date label CPC-UNI assigns, whereas the BMKG archive
dates each total to the local day ending at the morning
observation. To quantify the contribution of this
...
Because marginal correction introduces no rank reversals in the
daily series, the correlation moves only within sampling
uncertainty across the LSEQM+DL stages
(Section~\ref{sec:discussion-bound}). The diagnostic is
...
Two poolings are reported. The first is the full
BMKG record ($2001$ to $2021$), the span of the half-hourly archive
processed for the window diagnostic, which resolves the dependence of
the recovery on the satellite era; $178$ of the $180$ archived stations
carry valid half-hourly coverage and enter this diagnostic. The second
is the GPM era ($2015$ to $2021$), which gives the seasonal and
per-station climatology over the years in which IMERG resolves the
diurnal cycle. Timezone bands
...
The Python implementation is partitioned into twelve modules under
...
\texttt{qa\_framework} (Composite Quality Index assembly),
\texttt{station\_validation} (per-station extraction and pairing),
\texttt{taylor\_diagram} (Taylor-diagram generation), and
\texttt{visualisation} (QA and station-validation figure
generation). Each module is importable and exposes a small number
...
hyperparameters, and the per-method output filenames. The full
template, with every parameter referred to in this chapter set to
the value used for the reported results, is reproduced in
Appendix~\ref{app:config}, and
Appendix~\ref{app:repro} sets out the steps required to re-run
the pipeline from it.

\subsection{Notebook Workflow}

The pipeline is exposed through six numbered Jupyter
notebooks~\citep{ref:kluyver2016} under \texttt{notebooks/}: 01 (data acquisition), 02 (LSEQM+DL
bias correction), 03 (performance measurement), 04 (QA
framework), 05 (station validation), and 06 (visualisation).
...
\begin{sloppypar}
The analysis runs on Python~3.12; its environment is declared in
\texttt{environment.yml} with pinned versions for the core
scientific packages: array and numerical
computing with NumPy
\citep{ref:harris2020numpy} and SciPy \citep{ref:virtanen2020scipy},
labelled-array and tabular handling with xarray
\citep{ref:hoyer2017xarray} and pandas \citep{ref:mckinney2010pandas},
the cross-validation split for the GPD fit with scikit-learn
\citep{ref:pedregosa2011sklearn}, the CNN with TensorFlow
\citep{ref:tensorflow2015} and Keras \citep{ref:chollet2015keras},
the L-moment estimators with \texttt{lmoments3}, and the figures
with Matplotlib \citep{ref:hunter2007matplotlib}.
\end{sloppypar} The environment was tested on a Windows
...
Figure~\ref{fig:reproducibility} maps these five elements and the
data flow between them.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_thesis_03_reproducibility.png}
\caption[Reproducible implementation architecture of the LSEQM+DL framework]{Reproducible implementation architecture of the LSEQM+DL framework\label{fig:reproducibility}}
\end{figure}
