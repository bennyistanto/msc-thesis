...
% Sections: 3.1 Data and Domain, 3.2 Pipeline Architecture,
%           3.3 Per-stage Formulation (LS, EQM/Gamma, GPD tail, CNN),
%           3.4 Station-density-aware Blending,
%           3.5 Validation Protocol (BMKG out-of-sample),
...
% ============================================================
%
% Drafting position in workflow:
%   [Methods] -> LitRev v1 -> Results -> Discussion -> LitRev v2
%   -> Conclusion -> Introduction -> Front matter
%
% Source materials for this chapter:
%   - paper/PPS1691_..._compact_20260526.tex  (compressed methods)
%   - paper/PPS1691_..._20260522_seminar.tex  (master, 20pp)
%   - paper/PPS1591_..._mdpi.docx              (parallel manuscript)
%   - paper/additional_information.md          (deep technical material)
%   - docs/ Quarto site                        (reproducibility evidence)
%   - src/ Python codebase                     (implementation artefact)
%   - notebooks/ Jupyter                       (workflow artefact)
%
% Conventions:
%   - No em-dashes anywhere (hyphen-space or colon)
%   - No AI-language watchlist words
%   - Long-form abbreviation on first use, short form after
%   - MDPI_STATUS placeholder for the parallel manuscript citation
%   - Station density mask: design choice, NOT "key contribution"
%   - alpha=0.70, GPD 80th percentile, density_saturation=3: sensitivity caveat preserved
% ============================================================
...
LSEQM+DL, comprising Linear Scaling (LS), Empirical Quantile Mapping (EQM), Generalized Pareto Distribution (GPD), and Convolutional Neural Network (CNN) refinement that constitutes the Deep Learning (DL) stage (Section~\ref{sec:methods-pipeline}), the
station-density-aware blending design
(Section~\ref{sec:methods-blending}), the reproducible
implementation that supports independent verification
(Section~\ref{sec:methods-reproducibility}), the out-of-sample
validation framework (Section~\ref{sec:methods-validation}), and
the verification metric catalogue
(Section~\ref{sec:methods-metrics}). The algorithmic details
...
corrected. The CPC Unified Gauge-Based Analysis of Daily
...
stations to the 171 retained for the validation reported in
...
Convergence Zone \citep{ref:tan2016}. Daily-mean precipitation is
...
UTC+10), a detail that matters for daily aggregation because the
...
\citep{ref:ramadhan2022}. This spatial gradient motivates the
...
Run (IMERG-L) because its 14-hour latency fits within the
same-day decision window of operational services such as
...
Indonesian bounding box before processing.
...
Over Indonesia, approximately 180 contributing stations enter the
analysis, with the contributing network concentrated in Java and
Sumatra. CPC-UNI is documented to systematically undercatch heavy
rainfall, reflecting both the smoothing introduced by interpolation
...
The raw archive contains 180 stations with daily observations
from 2001-01-01 to 2025-12-31. Missing days are sentinel-coded as
$8888.0$ in the source file; after sentinel removal the archive
yields 880{,}702 valid daily observations. Two filters are
...
Together the two filters drop 9 of the 180 archived stations,
leaving 171 with usable data for the per-station verification
reported in Chapter~\ref{chap:results}. The 9 dropped stations
...
late in the 2001 to 2025 window. The retained network spans the
...
\caption[Spatial distribution of the 180 archived BMKG stations, color-coded by region]{Spatial distribution of the 180 archived BMKG stations,
color-coded by region. The legend reports per-region counts in
the archived network; 171 of the 180 are retained after spatial
and minimum-paired-day filtering. Province boundaries (light
gray) and the Indonesian national boundary (dark outline) are
overlaid; neighbor countries appear in light gray. The gauge
network is concentrated over Java and southern Sumatra, with
sparser coverage over Papua, Maluku, and eastern Nusa
Tenggara.\label{fig:station-map}}
\end{figure}
...
\caption[Conceptual workflow of the LSEQM+DL hybrid bias correction framework]{Conceptual workflow of the LSEQM+DL hybrid bias
correction framework. Each stage targets a distinct bias
dimension; the output of one stage is the input of the next. The
final stage blends the CNN refinement with the LSEQM result
using a station-density-aware weight (Section~\ref{sec:methods-blending}).\label{fig:pipeline}}
\end{figure}
...
The non-negativity constraint is enforced at post-processing
through the 99.9th-percentile cap from
Section~\ref{sec:methods-gpd} rather than at the activation.
Dropout in three places (after each of the two MaxPool layers
and after the Dense bottleneck) is aggressive regularisation
for a network this small; this is appropriate given the
per-dekad sample size, discussed below.
...
mean squared error computed at extreme pixels only, the Adam
...
Section~\ref{sec:methods-blending} formalises: the upper region
of the figure presents the Conv2D-MaxPool-Dropout backbone and
the Flatten-Dense reconstruction head described above, the
centre presents the alpha-blending equation that combines the
CNN output with the LSEQM intermediate, and the lower region
presents the station-density confidence-mask construction.
...
\caption[Stage~4 architecture in detail: the Convolutional Neural Network forward pass (top), the alpha-blending step]{Stage~4 architecture in detail: the Convolutional
Neural Network forward pass (top), the alpha-blending step that
combines the CNN refinement with the LSEQM intermediate
(centre), and the station-density confidence-mask construction
(bottom). The training configuration and key behavioural rules
are summarised in the sidebars. The blending and confidence-mask
construction are formalised in
Section~\ref{sec:methods-blending}.\label{fig:cnn-arch}}
\end{figure}

Figure~\ref{fig:stage-demo} shows the effect of the four stages
on an illustrative 60-day sample, arranged in a $3 \times 3$
grid. The reference shown in the figure is the CPC-UNI
calibration target the pipeline acts on; the independent
out-of-sample validation against 171 BMKG stations is reported
in Chapter~\ref{chap:results}. The first row walks through the
early stages on the time-series view and includes a dedicated
mechanism panel for quantile mapping; the second row collects
distribution diagnostics across the EQM and GPD stages; the
third row covers the CNN refinement, the final corrected output,
and the headline metric summary. The data are synthetic for
...
\caption[Illustrative effect of the LSEQM+DL pipeline stages applied to a synthetic 60-day daily-precipitation sample]{Illustrative effect of the LSEQM+DL pipeline stages
applied to a synthetic 60-day daily-precipitation sample. The
reference shown throughout panels (a) through (h) is the CPC-UNI
calibration target; independent out-of-sample validation against
171 BMKG stations is reported in Chapter~\ref{chap:results}.
(a)~Raw IMERG against the CPC-UNI reference: the satellite
over-detects light rain and under-shoots heavy events.
(b)~After Stage~1 LS: dekadal means match. (c)~Stage~2 EQM
mechanism: the LS-source and CPC-UNI target Gamma cumulative
distribution functions plotted on the same axis, with arrows at
selected quantiles (0.40, 0.70, 0.92) marking the
source-to-target value mapping that defines quantile mapping.
(d)~After Stage~2 EQM applied: the marginal distribution aligns
with the reference in the time-series view. (e)~Stage~3 GPD:
wet-day quantile profile showing the EQM-only fit (blue squares)
staying short of the reference tail and the GPD-lifted profile
(amber triangles) recovering the upper tail above the
80th-percentile threshold. (f)~Wet-day empirical cumulative
distribution functions overlaid across all stages, showing
convergence toward the CPC-UNI CDF. (g)~Stage~4 CNN refinement
at extreme pixels (scatter against the LSEQM intermediate value,
blended at $\alpha = 0.70$). (h)~Final LSEQM+DL output against
the CPC-UNI reference. (i)~Deviation of relative bias,
$\mathrm{SDR} - 1$, and $Q_{99}$ ratio $- 1$ from the target
value of zero at each successive stage. Synthetic data; empirical
out-of-sample values against BMKG appear in
Chapter~\ref{chap:results}.\label{fig:stage-demo}}
\end{figure}
...
the stations that contribute to the CPC-UNI gauge analysis are
counted within each $0.5^{\circ}$ CPC cell. The relevant
network is the CPC-UNI contributing-station list, which
controls how well the reference is constrained at each cell; it
is not the 171-station BMKG validation network of
Section~\ref{sec:methods-data}, which serves a separate purpose
in out-of-sample verification. The two networks overlap
substantially but are not identical. Second, the count field is smoothed with
...
JupyterLab; the behaviour is the same because the configuration
...
The second tier compares the corrected satellite against the 171
...
across the 171 stations and the 36 dekadal windows unless stated
...
addition, multi-threshold verification at seven thresholds
(1, 5, 10, 20, 50, 100, and 150 mm/day) reports the categorical
skill of the corrected series following the World Meteorological
Organization (WMO) verification standard TD-1485
\citep{ref:wmo1485}. At each threshold, hits $H$, false alarms
...
through a Quarto site. Validation against 171 out-of-sample BMKG stations
...
% END OF SKELETON
%
% Estimated total prose word count when filled: ~4,500-6,500 words
% Estimated final page count: 15-20 pages
%
% Cross-references this chapter creates (Sections that reference it):
%   - Section 4.1, 4.2, 4.3 (Results, headline skill) <- Section 3.6
%   - Section 4.8 (Sensitivity analyses) <- Section 3.2 (alpha, GPD),
%     Section 3.3 (saturation_count)
%   - Section 4.9 (Reproducibility evaluation) <- Section 3.4
%   - Section 5.1 (Theoretical bound) <- Section 3.6 (Pearson r as
%     separate diagnostic, framing it for the paradox)
%
% Cross-references this chapter expects from Lit Review:
%   - Section 2.1 (IMERG products) -> 3.1 (data)
%   - Section 2.3 (bias correction methods) -> 3.2 (pipeline stages)
%   - Section 2.5 (reproducibility) -> 3.4 (implementation)
%
% ============================================================
