...
framework against 172 independent Indonesian Meteorological, Climatological, and Geophysical Agency (BMKG) stations, pooled across
...
detection skill. Skill is broken out at the World Meteorological Organization (WMO)
multi-threshold verification thresholds in
Figure~\ref{fig:threshold-skill}. These follow the WMO
classifications \citep{ref:wmo1132}: $1$~mm (measurable), $5$~mm
(light), $10$~mm (moderate), $20$~mm (moderate-heavy), $50$~mm
(heavy), and $100$~mm (very heavy). The $150$~mm/day extreme class
is omitted because no station reaches the minimum of ten
exceedances required for stable categorical verification in any
dekad of the record. At the
...
against $0.062$ for LS, a $56\%$ improvement, and the equitable
threat score (ETS), which removes the chance-hit contribution,
shows the same ordering. At the $100~\mathrm{mm/day}$
threshold the absolute CSI values are small for both methods,
reflecting the difficulty of detecting individual heavy-rain events
at the daily scale, but the LSEQM+DL advantage is consistent across
thresholds.
...
\caption[Threshold-stratified verification at the $172$ BMKG station locations]{Threshold-stratified verification at the $172$ BMKG
...
not improve across the four correction stages: $r$ stays near
...
was fitted to: one amplitude metric improves substantially while
three temporal-skill metrics do not move or get worse.
...
product; the offset search follows the gauge-timing procedure of
\citet{ref:beck2019}. The diagnostic pools the $178$ of the $180$
...
full-record headline of $r \approx 0.34$
(Section~\ref{sec:results-correlation}) because the full record
blends the GPM era with the earlier, timing-imprecise era, a point
Section~\ref{sec:discussion-subdaily} returns to. The recovery is
...
(Figure~\ref{fig:window-seasonal}), and the optimum does not
migrate between the wet and dry monsoon phases.

\begin{figure}[H]
\centering
\includegraphics[width=\figFourTenWidth\textwidth]{fig_thesis_04_window_seasonal.png}
\caption[GPM-era window-offset correlation and seasonal stability]{GPM-era
window-offset correlation by timezone band (a) and the
season-invariance of the peak offset~$h^{\star}$ across the twelve
three-month running seasons (b).\label{fig:window-seasonal}}
\end{figure}
...
(Figure~\ref{fig:window-map}). The recovered per-station
correlation has a median of $0.56$, an interquartile range of
$0.51$ to $0.62$, and a maximum near $0.78$; pooled within each
timezone band at the matched window it ranges from $0.56$ for WIB
to $0.62$ for WIT. The offset is therefore a property of the gauge
reporting convention shared across the network, not a local effect.

\begin{figure}[H]
\centering
\includegraphics[width=\figFourElevenWidth\textwidth]{fig_thesis_04_window_map.png}
\caption[Per-station optimal window offset and correlation lift]{Per-station
peak offset~$h^{\star}$ across Indonesia (a) and the per-station
correlation lift from the UTC-day to the matched window, ordered by
longitude and coloured by timezone band (b).\label{fig:window-map}}
\end{figure}
...
These results localise the unchanged correlation of
...
of the $172$ validation stations.
...
fifteen settings requires a complete end-to-end pipeline run,
including per-dekad CNN training: the full sweep is tractable on the
$126$-pixel Bali grid (about $72$ minutes per run;
Section~\ref{sec:results-reproducibility}), but the per-pixel stages
scale with the pixel count, and for the $10^{4}$ to $10^{5}$ land
...
daily values stays near $0.34$ across all stages, station-by-station,
and month-by-month. The spatial CQI maps concentrate the skill in the
densely gauged west, with graceful degradation to the LSEQM baseline
in the sparsely gauged east, and the pipeline runs end-to-end on a
free Colab in $72.1$ minutes on the Bali subdomain.
