...
(Section~\ref{sec:discussion-bound}). The CNN thus acts where the
statistical stages leave a spatially incoherent residual in the
extremes, which is the dimension it was placed at the end of the
pipeline to address, without disturbing the bulk field that
Stages~1 to 3 have already corrected.
...
threshold because it declares rain on more days. The distribution
step of LSEQM+DL removes that over-detection, reclassifying the
spurious drizzle to dry, which costs some of those low-threshold
hits. The reversal does not occur at $5~\mathrm{mm/day}$: LS retains
the higher probability of detection there and through
$10~\mathrm{mm/day}$, and the ordering changes only at
$20~\mathrm{mm/day}$.
...
methods. The advantage is therefore established at the $20$ and
$50~\mathrm{mm/day}$ thresholds, which is where the record carries
enough events to resolve it.
...
below \citep{ref:peterson2001, ref:zhang2011}. The regime in which LSEQM+DL
overtakes LS is therefore the regime that internationally
standardised indices class as very heavy precipitation, which is
the regime that threshold-based warning and monitoring
applications over the Maritime Continent depend on
...
reports the same result against the in-sample CPC-UNI target: the
two are the two references of a single finding, and together they
are what the discussion develops as the central observation of the
thesis.
...
excludes the two preparatory notebooks: defining the area of
interest and downloading the source archive are dominated by the
extent of the domain requested and by network throughput, so
neither yields a timing that another user could reproduce or
usefully compare.
