...
for stable categorical verification in any dekad of the record. At the
...
$0.34$ across all three, RMSE rises from $13.1$ to $14.1$~mm/day
...
Section~\ref{sec:results-correlation}. The era difference dominates,
a point Section~\ref{sec:discussion-subdaily} returns to. The recovery is
...
falling below $-0.01$ over none). The CQI map places the product in
the Fair band ($0.40$ to $0.60$) across $98.5\%$ of the land mass
(mean $0.503$), with a west-east gradient that tracks gauge
density: higher within the Fair band over Java, Sumatra, and the
well-observed interiors, and dropping into the Poor tier over the
...
\caption[CQI change and continuous quality for LSEQM+DL]{(a) Per-pixel CQI change from LSEQM to LSEQM+DL; (b) LSEQM+DL CQI, within the Fair tier (0.40 to 0.60) almost everywhere; both panels spatially smoothed.\label{fig:cqi-improvement}}
\end{figure}
