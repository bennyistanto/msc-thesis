...
\begin{table}[H]
\centering
\caption[Stage-wise skill against 171 independent BMKG stations]{Stage-wise skill against 171 independent BMKG stations,
pooled across all 36 dekads (2001 to 2025).\label{tab:headline}}
\begingroup\small
...
perfect agreement with observations. The parallel manuscript
...
\caption[Stage-wise skill at 171 BMKG stations for four representative metrics]{Stage-wise skill at 171 independent BMKG stations for
...
150~mm/day) in Figure~\ref{fig:threshold-skill}. These thresholds
follow the WMO classifications \citep{ref:wmo1132}: $1$~mm
(measurable), $5$~mm (light), $10$~mm (moderate), $20$~mm
(moderate-heavy), $50$~mm (heavy), and $100$~mm (very heavy). At the
...
quality tiers (panel~b) that run from Poor ($<0.3$) to Excellent
($\geq 0.7$). The LSEQM+DL improvement is
...
\caption[Per-pixel CQI change from LSEQM to LSEQM+DL]{(a) Per-pixel CQI change from LSEQM to LSEQM+DL,
smoothed spatially for clarity.\label{fig:cqi-improvement}}
\end{figure}
...
metrics across the three correction stages.\label{fig:temporal-stability}}
\end{figure}
...
parameters identified in Chapter~\ref{chap:methods}.\label{tab:sensitivity-bali}}
\begingroup\small
...
subdomain and its data coverage, and this self-contained domain
is the one timed in Table~\ref{tab:colab-runtime}.

\begin{figure}[H]
\centering
\includegraphics[width=\figFourEightWidth\textwidth]{fig_thesis_04_bali_coverage.png}
\caption[The Bali reproducibility subdomain]{The Bali reproducibility subdomain.\label{fig:bali-coverage}}
\end{figure}
...
\begin{table}[H]
\centering
\caption[Measured end-to-end runtime of the LSEQM+DL pipeline on Google Colab]{Measured end-to-end wall-clock runtime of the LSEQM+DL
...
Against $171$ independent BMKG stations, the LSEQM+DL framework
moves three of the four verification pillars cleanly toward the
gauge target and produces a designed trade-off in the fourth. Value adjustment, distribution
...
lower false-alarm ratio. The stage-wise attribution confirms that each
...
minutes for the Bali subdomain across all dekads. Chapter~\ref{chap:discussion} now explains
this flat correlation as a structural property of marginal
correction, not a failure of the framework.
