...
A specific caveat applies in the Maritime Continent. The
convective diurnal cycle over Indonesia peaks at local 14:00 to
17:00, which falls in the UTC 07:00 to 10:00 band for the western
archipelago and the UTC 06:00 to 09:00 band for the eastern
archipelago. The daily aggregation window used by the present
pipeline is aligned to UTC and therefore splits the local
convective afternoon across two calendar days for stations east
of UTC+7. The Pearson $r$ ceiling reported in
Chapter~\ref{chap:results} mixes the underlying satellite timing
accuracy with this calendar artefact. The implication for
operational users is that the $r \approx 0.34$ figure should be
read as a lower bound on what a locally-aligned daily product
might achieve, and that the gap between the lower bound and the
locally-aligned value is recoverable by re-aggregation rather
than by a fundamentally different correction.
...
in the UTC+7 to UTC+9 band, is local-day re-aggregation. The
calendar artefact discussed in
Section~\ref{sec:discussion-ops} is a fixable mismatch between
the UTC-aligned daily window and the local convective
afternoon. Re-aggregating the native half-hourly IMERG fields
to a local-day window aligned to the convective cycle, before
applying the LSEQM+DL correction, would remove the artefact
without changing the correction methodology. The cost is a
re-run of the entire pipeline against a re-aggregated input
archive, which is computationally modest but archive-intensive.
The Himawari-9 cloud-tracking reference makes the diurnal cycle
visible at $10$-minute resolution and would let an analyst
verify the local-day alignment against the satellite-observed
peak in convective cloud cover. This is the path most directly
enabled by the parallel reproducibility contribution: the
configurable daily-aggregation step is exposed in the pipeline,
the re-aggregated archive can be substituted at the input layer,
and the four-pillar verification report on the re-aggregated
pipeline against the same BMKG validation set is reproducible by
any independent user.
...
The present work has five limitations that bound the strength of
...
The combined effect of these five limitations leaves the central
