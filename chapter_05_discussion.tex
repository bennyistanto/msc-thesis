...
The bound is empirically supported by locating $r \approx 0.34$
inside the $0.3$ to $0.5$ range documented for daily IMERG over
the tropics, which rises substantially only at dekadal or monthly
aggregation (Section~\ref{sec:lit-bounds}). The corrected
LSEQM+DL value of $0.348$ sits inside this empirical band. The ceiling has two
...
morning-observation label, so against BMKG the daily correlation
is suppressed under UTC alignment and recovered by re-aggregating
...
significance of this split. The pattern is
...
low daily correlation is largely a fixable metadata effect:
re-aggregating the satellite to the local-observation day lifts
the GPM-era daily correlation from approximately $0.20$ to
approximately $0.57$, whereas the CPC-UNI calibration label
already coincides with the UTC day and needs no such offset. The
...
threshold reliability diagrams for event detection~\citep{ref:wilks2011}. The path is
...
rainfall and $0.5^{\circ}$ interpolation smoothing
(Chapter~\ref{chap:methods}) bound the reference quality. The
...
the UTC calendar day, while both CPC-UNI and the BMKG network
follow the local-day morning-observation convention of their
contributing stations \citep{ref:cpcuni_readme, ref:bmkg2006sk32},
a mismatch of approximately $23$~hours per the diagnostic in
Section~\ref{sec:discussion-subdaily}. The four-pillar results
