...
the morning-observation BMKG gauge convention.
...
contributors: the timing accuracy of the underlying retrieval,
which lies upstream of every step in the pipeline and is the
dominant component; and the UTC-aligned daily aggregation window,
which splits the local convective afternoon across two calendar
days for stations east of UTC+7 and is therefore a fixable
calendar artefact rather than a fundamental satellite limit.
...
therefore lift the operational Pearson~$r$ from approximately
$0.34$ to approximately $0.62$ without changing any other
pipeline output. The diagnostic is recorded here as a
...
IMERG-L window is aligned to UTC, and the BMKG gauge daily total
is aligned to the morning-observation convention defined by Head
of BMKG Decree SK.32/TL.202/KB/BMG-2006 \citep{ref:bmkg2006sk32}.
...
matching the BMKG morning-observation convention
\citep{ref:bmkg2006sk32}. Re-aggregating the native half-hourly
...
\textbf{Validation window mismatch with the BMKG gauge convention.}
The Pearson~$r$ in Table~\ref{tab:headline} is computed between
the UTC-day IMERG-L aggregate and the morning-observation BMKG
gauge daily total, two products whose 24-hour windows are offset
by approximately $22$~hours per the diagnostic in
Section~\ref{sec:discussion-subdaily}. The diagnostic shows that
this metadata mismatch alone accounts for approximately
two-thirds of the gap between the observed $r \approx 0.34$ and
the window-aligned upper bound of $r \approx 0.62$. The four-pillar
results (value, distribution, extreme, detection) are unaffected
because they are functions of the marginal distribution and of
categorical events, both invariant to the 24-hour window choice.
Re-aggregating the daily IMERG-L archive to a morning-to-morning
window matched to the BMKG convention is identified as a path
forward (Section~\ref{sec:discussion-paths}).
