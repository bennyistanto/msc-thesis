...
% Sections: 2.1 Satellite-derived Daily Precipitation,
%           2.2 Bias Dimensions and Verification,
%           2.3 Bias Correction Method Families,
%           2.4 Theoretical Bounds on Marginal Correction,
%           2.5 Maritime Continent Precipitation Context,
%           2.6 Reproducible Computational Notebook Workflows in Climate and
%           Earth Observation Analytics, 2.7 Chapter summary.
...
refinement, a Deep Learning (DL) architecture) framework introduced 
...
LSEQM+DL framework draws. Three further sections complete the
chapter: Section~\ref{sec:lit-bounds} discusses the theoretical
limits of marginal-distribution correction methods on
temporal-skill metrics; Section~\ref{sec:lit-maritime-continent}
reviews the regional precipitation context of the Indonesian
Maritime Continent; and Section~\ref{sec:lit-reproducibility}
reviews the state of reproducibility in climate and hydrology
software. Together these six sections frame the methodological
...
the contributing gauge network is spatially uneven, dense in the
populated west and sparse over the eastern archipelago~\citep{ref:chen2008}. The
...
The IMERG algorithm~\citep{ref:huffman2020} merges two
observational streams and a gauge-anchoring pass.
...
between successive overpasses \citep{ref:joyce2004}, producing a continuous
...
\textit{Late Run} (IMERG-L)~\citep{ref:imergdl07} is released
approximately 14~hours after real time. The
\textit{Final Run}~\citep{ref:imergdf07} is released
approximately 3.5 months after real time and benefits from the
monthly gauge anchoring step
...
tropics, where latency is the typical time lag between
observation and product release and gauge integration describes
whether and how gauge data is incorporated in the product.

CPC Unified Gauge-Based Analysis of Global Daily Precipitation
(CPC-UNI) \citep{ref:chen2008} interpolates gauge
...
0.1$^{\circ}$ grid but differ in latency and in whether the
monthly gauge anchoring step is applied (Final only).
...
at 0.1$^{\circ}$. Regional efforts have also calibrated IMERG
directly against a daily gauge analysis: the AIMERG dataset
re-calibrates GPM-era IMERG at the daily scale against the
APHRODITE analysis over monsoon Asia \citep{ref:ma2020}.

The product selected for correction here, and the reasoning behind
it, are given in Section~\ref{sec:methods-data}.
...
tropical hydrological and operational applications\label{tab:gridded-products}}
...
IMERG Final Run         & 3.5 months & 2000 -- present & Ensemble with monthly gauges    & 0.1$^{\circ}$ \\
...
gauge networks have documented three persistent bias modes. The
first is over-detection of light rainfall, where the satellite
reports rain when gauges report dry, often associated with
sub-pixel cloud artefacts or shallow drizzle that the satellite
retrieves but the gauge misses. The second is under-detection of
heavy events, where the satellite under-estimates extreme
rainfall magnitudes, a limitation of microwave retrieval at the
upper tail of precipitation rates. The third is a full
distributional mismatch between the satellite and gauge
distributions of daily
precipitation~\citep{ref:maggioni2016, ref:tan2016, ref:tang2020}.
These bias modes motivate the methodological
families reviewed in Section~\ref{sec:lit-methods}; the
empirical magnitudes for IMERG-L over Indonesia are reported in
Chapter~\ref{chap:results}.

\subsection{Quality of Gauge-Based Analyses}\label{sec:lit-gauge-quality}

A gauge-based analysis is the natural reference against which a
satellite product is corrected, but it is an interpolated field
rather than a direct observation, and its quality is not uniform.
Three properties documented in the literature govern how far such
an analysis can be trusted as a calibration target.

The first is the size of the station population that reaches it.
Daily gauge reports are exchanged internationally over the World
Meteorological Organization Global Telecommunication System (GTS)
\citep{ref:chen2006gtsqc, ref:chen2008}, and that stream is a
subset of what national services operate: approximately $8{,}000$
of the $12{,}000$ stations listed in WMO Volume A are exchanged
on a regular basis, against more than $85{,}000$ stations in the
offline archives assembled retrospectively \citep{ref:becker2013}.
Denser regional analyses exist because that stream alone is thin.
The global merged products in routine use rest on about $6{,}000$
stations worldwide reporting over the GTS, so the APHRODITE
analysis for Asia had to be assembled from three sources: the GTS
stream, data precompiled by other projects, and its own collection,
for which access had to be negotiated with national meteorological
and hydrological services \citep{ref:yatagai2012}.

The second is that analysis quality tracks network density. The
CPC-UNI product documentation records that quality varies with
gauge network density and directs users to the per-cell gauge
count distributed with each daily file \citep{ref:cpcuni_readme}.
The algorithm paper quantifies the relationship, reporting a
grid-cell correlation below $0.6$ where the nearest contributing
station is more than $150$~km away against $0.9$ or higher where a
report is available within $50$~km \citep{ref:xie2007}, and the
cross-validation of the objective technique degrades as the
network thins \citep{ref:chen2008}. A controlled comparison across
station densities and grid resolutions finds density to be the
dominant control on the quality of a gauge-based grid, ahead of
the grid spacing itself \citep{ref:merino2021}.

The third is that interpolation over a sparse network compresses
the distribution at both ends, reducing the frequency of both the
highest and the no-rain amounts relative to the station
observations \citep{ref:xie2007}. Over Indonesia specifically, an
evaluation co-authored by BMKG scientists found the high spatial
variability of maritime-continent rainfall a challenge for
interpolation from on-ground gauges, and could assemble only $76$
stations with $75\%$ monthly completeness across its six
validation areas \citep{ref:vernimmen2012}. Together these three
properties are what
Section~\ref{sec:methods-reference-quality} applies to the
specific choice of calibration reference made in this study.
...
hides the dimension in which the product fails
\citep{ref:murphy1993, ref:ebert2007}. The verification
...
A four-pillar grouping of verification
metrics separates \textit{value adjustment} (mean offset and spread
...
\citep{ref:ebert2007, ref:wilks2020} and matches the World
Meteorological Organization (WMO) verification standards for
quantitative precipitation forecasts~\citep{ref:wmo1485, ref:wmo1132}. 
The framework allows the question \enquote{how well does this product 
reproduce the gauge field?} to be decomposed into four narrower questions,
...
marginal distributions \citep{ref:gupta2009}. The distinction between
...
pinned close to its raw value is developed in
Section~\ref{sec:lit-bounds}.
...
score~\citep{ref:wilks2020}. Single-threshold reporting hides the
...
exposing the threshold-dependence directly, and the WMO principle
is that the extreme-precipitation thresholds themselves are set
regionally~\citep{ref:wmo1310}. The threshold set used here
follows from both and is listed in
...
sites~\citep{ref:adam2003}. Gridded gauge analyses inherit the
...
\subsection{Accumulation-Window and Time-of-Observation Effects}\label{sec:lit-window}

A second property of the verification reference, distinct from its
spatial biases, is the time window over which each daily total is
accumulated. Satellite products are distributed on the Coordinated
Universal Time (UTC) calendar day, whereas rain gauges report under
national time-of-observation conventions, typically a 24-hour total
ending at a fixed morning observation in local time, following the
World Meteorological Organization principle of common observation
times \citep{ref:wmo2018no8}. When daily satellite and gauge values
labelled with the same calendar date refer to different 24-hour
periods, the day-by-day correlation is suppressed even when the two
products describe the same rainfall.

This dependence on temporal matching is documented in the
validation literature. The IMERG diurnal cycle is meaningful only
in local solar time, and satellite-gauge agreement degrades rapidly
when the two are compared at mismatched times \citep{ref:li2018, ref:tan2019}. Where the accumulation window has been aligned to the
gauge convention, the agreement improves: evaluations over China
and over southern South America re-aggregate the sub-daily satellite
field to a $12$~UTC-to-$12$~UTC window to match the national daily
reporting time before computing skill \citep{ref:salio2015, ref:xie2022}. The global MSWEP analysis infers each gauge's
reporting time by shifting the accumulation window to maximise
correlation against independent products \citep{ref:beck2019}.

This correction has been adopted unevenly, and in particular it has
rarely been propagated from validation into daily bias correction.
Over the Indonesian Maritime Continent the multi-scale evaluation of
IMERG against gauges states that both series were processed in
Coordinated Universal Time \citep{ref:ramadhan2022ts}, so the
gauge reporting day is not matched, and daily bias-correction
studies likewise adjust rainfall magnitudes against a fixed daily
window \citep{ref:ramadhan2024sumatra, ref:suhartanto2025, ref:wang2026}.
The window contribution to the low daily correlation is therefore
left unquantified. This
gap motivates the window-offset diagnostic developed in
Section~\ref{sec:methods-window} and the recovery reported in
Section~\ref{sec:results-window}.
...
correspond directly to the four stages of the LSEQM+DL framework:
...
\citet{ref:maraun2016}.

Each family contains many variants beyond the representative method
named here. The scaling family includes the delta or change-factor
method, local intensity scaling, and power transformation, of which
linear scaling is the simplest multiplicative case; the
quantile-mapping family includes empirical and parametric mapping,
quantile delta mapping, scaled distribution mapping, and
cumulative-distribution-function transformation
\citep{ref:gudmundsson2012, ref:teutschbein2012, ref:cannon2015}. This
review selects one canonical representative per family rather than
surveying every variant, because the contribution of this thesis is
the staged composition of the four families into a single daily
pipeline, not a comparison among variants of any one of them. Variants
developed primarily to preserve long-term trends in climate-projection
downscaling, such as quantile delta mapping, are not central to the
satellite-versus-gauge setting, where there is no projected trend to
preserve.
...
the LSEQM+DL framework of Chapter~\ref{chap:methods} can be
positioned. The table also includes three methods the framework
does not adopt (Bias Correction and Spatial Disaggregation,
Bayesian Model Averaging, and the Censored Shifted Mixture
Distribution), listed as established alternatives for daily
precipitation that fall outside its one-dimension-per-stage
design.
...
considered for daily precipitation\label{tab:biascorr-methods}}
\begingroup\footnotesize
...
uses the multiplicative form, rescaling the satellite series by
the ratio of the reference mean to the satellite mean, which
preserves non-negativity \citep{ref:teutschbein2012}. The means
are computed over a calibration window matched
to the temporal scale at which the bias is assumed stationary
(monthly, dekadal, or seasonal); the form adopted here is stated
in the notation of this study as
Equation~\eqref{eq:ls}. LS corrects the first moment
...
satellite value is evaluated on its own cumulative distribution
function (CDF) and the resulting non-exceedance probability is
inverted through the reference CDF
(Equation~\eqref{eq:eqm}). Two main variants of QM differ in how the
...
for sample-size efficiency \citep{ref:gudmundsson2012}.
...
\citep{ref:wilks2020}. The EQM
variant with a Gamma fit, adopted in Stage~2 of the LSEQM+DL
framework, traces to the bias correction methodology of
\citet{ref:gudmundsson2012, ref:themessl2012} and is the
standard reference implementation for daily precipitation
correction in climate downscaling contexts. In the
satellite-precipitation setting, the same quantile mapping has
been applied with a gauge reference across heterogeneous climate
zones, for example over Iran using regionally pooled CDFs to
stabilise the mapping where gauges are sparse
\citep{ref:katiraie2020}.
...
correction of \citet{ref:cannon2015} preserves the gauge wet-day
...
threshold converges to a GPD~\citep{ref:pickands1975, ref:coles2001}.
Its shape parameter $\xi$ governs tail heaviness, taking positive
values for the heavy Pareto-like tails typical of tropical
convective extremes.
...
above the threshold. Hybrid schemes of this kind, pairing a
quantile mapping for the bulk with a distinct correction for the
upper tail, improve the fidelity of precipitation extremes
relative to a single global mapping \citep{ref:holthuijzen2022}.

The threshold is typically chosen as the
90th to 99th percentile of the wet-day sample, with the trade-off
...
\citep{ref:hoskingwallis1997}. The estimator and threshold
adopted in this study, and the reasoning behind them, are given in
...
\citep{ref:reichstein2019}.

Convolutional Neural Networks (CNNs) have been applied to
precipitation estimation and statistical downscaling in several
recent studies~\citep{ref:pan2019, ref:banomedina2020}.
Deep-learning bias correction of satellite precipitation
specifically has used recurrent networks for tropical daily
rainfall \citep{ref:yang2022}, convolutional and autoencoder
architectures for daily fields \citep{ref:le2020}, multi-task
networks that jointly correct and merge \citep{ref:jiao2025}, and
architectures that fold in terrain, including U-Net variants over
complex orography \citep{ref:dao2025}. The principle
...
CNNs to satellite precipitation correction \citep{ref:banomedina2020}. First, restricting
...
where the CNN has not been adequately trained.
Section~\ref{sec:methods-cnn} describes how both choices are
implemented here.
...
the literature. The LSEQM+DL framework described in
...
the verification literature has examined closely
\citep{ref:maraun2016}: they correct
...
the values they act on. This section reviews the two established
results that together bound the time-dependent skill attainable by
any marginal correction, and that frame the central empirical
...
distribution. When the mapping is monotone and non-decreasing, which
...
preserved. If the source series reports a higher value
on one day than on another, the corrected series reports a value
no lower on that day too
\citep{ref:cannon2015}. Cannon and colleagues examined this in the
context of trend preservation, showing that quantile mapping can
distort climate-change trends precisely because it operates on
ranks rather than on the temporal sequence; the same mechanism
fixes the day-by-day ordering of a corrected series relative to its
source.
...
with which gauge day, and the day-by-day correlation is therefore
held close to the correlation between the raw satellite series and
the gauge. The amplitude correction that moves the standard
deviation ratio toward unity has no corresponding effect on that
pairing. This is not a deficiency
...
it \citep{ref:maraun2013, ref:cannon2015}.

The empirical magnitude of the bound is documented across regions,
and it varies widely with the setting. A multi-product comparison
against the dense Chinese gauge analysis reports median daily
correlations near $0.75$ for the two IMERG products, computed per
$0.25^{\circ}$ grid cell over 2000 to 2018, with the other satellite
and reanalysis products spanning roughly $0.45$ to $0.81$
\citep[Fig.~3]{ref:tang2020}. Evaluations over the Indonesian
Maritime Continent report daily correlations that rise substantially
when the aggregation window is widened to dekadal or monthly scales
\citep{ref:ramadhan2022ts}. The spread across these settings is
itself the point: the value obtained depends on the reference, the
density of the gauge network behind it, the
aggregation window and the calendar label used to pair the series,
and Chapter~\ref{chap:results} reports values across a comparable
range under different constructions. The present study tests
the rank-preservation bound directly by reporting the Pearson
correlation at each correction stage against the CPC-UNI
calibration target and, out of sample, against the independent BMKG
stations (Chapter~\ref{chap:results}), and develops its structural
...
satellite estimation \citep{ref:yamanaka2016}, and several of its
features bear directly on the verification and correction problem
...
land \citep{ref:mori2004, ref:wu2008}. A validation of IMERG against
$302$ automatic gauges across the region reproduces the diurnal
amount and frequency well, but places the satellite peak in the
early afternoon where the gauges place it in the early morning,
a phase error that a daily aggregate conceals
\citep{ref:ramadhan2024diurnal}. Indonesia spans three standard
time zones, from UTC+7 in the west (Sumatra, Western Kalimantan,
Java) through UTC+8 (Bali, Nusa Tenggara, Eastern Kalimantan,
Sulawesi) to UTC+9 in the east (Maluku, Papua). 
The IMERG product is accumulated on a UTC calendar day,
...
The convective peak therefore sits well inside a single UTC day, so
the effect is one of labelling rather than of splitting: in the western zone
the 07:00 local morning observation coincides with 00:00~UTC, so a
gauge total closed at that reading spans a complete UTC day but
carries the following calendar date. The afternoon convection of
one UTC day is thus reported under the next day's gauge label, the
region-specific form of the accumulation-window effect
introduced in Section~\ref{sec:lit-window}. Because the two
conventions differ by a whole day rather than by a partial
overlap, the mismatch is correctable by a date shift, which is the
path Chapter~\ref{chap:discussion}
identifies as the most actionable next step for the region.

Gauge coverage over the archipelago is strongly heterogeneous in
space, and two distinct networks are involved. The Indonesian
Meteorological, Climatological, and Geophysical Agency (BMKG)
operates the national station network used here for independent
validation, which is dense over Java and southern
Sumatra, where population and agricultural value are
concentrated, and progressively sparser eastward into Maluku,
Nusa Tenggara, and Papua. A subset of those stations reaches the
international exchange that feeds CPC-UNI, so the analysis is
constrained by fewer sites than the national network contains.
Because CPC-UNI is
a gauge-interpolated analysis, its quality follows the same
gradient \citep{ref:xie2007, ref:chen2008}: the reference is well constrained in the west and
...
kilometre-scale spatial resolution \citep{ref:bessho2016}, which is
sufficient to track
the convective afternoon and to align a re-aggregated daily
window to the local cycle. This study does not perform that
re-aggregation; Chapter~\ref{chap:discussion} proposes it as the
natural extension.
...
re-implement the method from the paper alone. This study adopts an
explicit reproducibility design, reviewed in
Section~\ref{sec:methods-reproducibility} and demonstrated in
Section~\ref{sec:results-reproducibility}; the literature it draws
on is reviewed below.
...
and Earth observation \cite{ref:perkel2018, ref:wagemann2022}. 
A notebook integrates code, results, and
...
ecosystem and is the format adopted here; it is portable across
execution environments, opening in either a local installation or a
...
self-documenting workflows \citep{ref:kluyver2016, ref:gentemann2021},
...
\citep{ref:yin2019, ref:gentemann2021}.

The combination of
computational notebooks and cloud platforms is also a teaching
aid: \citet{ref:haedrich2023} use notebook-based tutorials in the
...
and \citet{ref:boeing2021} argue that the notebook format makes
geographic analysis more nimble and extensible than conventional
Graphical User Interface (GUI)-driven workflows.

Two properties of the format are decisive for a reproducibility
design. A notebook can expose each analytical step as a runnable
artefact tied to specific functions in the underlying codebase, so
a reader can move from the documentation to the runnable cell and
back \cite{ref:perkel2018, ref:granger2021, ref:beg2021}. The same
file also opens in either a local installation or a cloud-hosted
session, making a workflow portable between the environment its
author used and the one a reader is likely to start with. How this
study applies both is described in
Section~\ref{sec:methods-reproducibility}, and the empirical
demonstration is reported in
...
rule. A reader who wishes to compare the LSEQM+DL framework of
...
independent reader. The implementation reported in
...
define the four stages that the LSEQM+DL framework composes. The
...
move: it can align the marginal distribution, but it leaves the
day-by-day correlation pinned close to the value set by the raw
satellite pairing, wobbling only within sampling noise.

The regional context of
Section~\ref{sec:lit-maritime-continent} establishes the
convective regime, the UTC calendar artefact, and the gauge
density gradient that shape both the correction design and its
interpretation. The reproducibility literature of
Section~\ref{sec:lit-reproducibility} establishes the
methodological expectation of independent verifiability that the
study's open framework is designed to meet. Chapter~\ref{chap:methods} now describes the
