...
%           Bounds on Marginal Correction, 2.5 Maritime Continent Regional
%           Context, 2.6 Reproducible Computational Notebook Workflows.
...
monthly gauge anchoring step is applied (Final only). The Climate Hazards
...
APHRODITE analysis over monsoon Asia \citep{ref:ma2020}. For the
present study, IMERG Late Run was
...
precipitation~\citep{ref:maggioni2016, ref:tang2020, ref:tan2016}.
...
in this study reports separate categorical metrics at seven
thresholds: the first five (1, 5, 10, 20, and 50~mm/day) follow
the WMO range, while 100 and 150~mm/day are added as heavy and
extreme classes for the Indonesian wet-season regime, consistent
with the WMO principle that extreme-precipitation thresholds are
set regionally~\citep{ref:wmo1310}.
...
when the two are compared at mismatched times \citep{ref:tan2019,
ref:li2018}. Where the accumulation window has been aligned to the
...
reporting time before computing skill \citep{ref:xie2022,
ref:salio2015}, and the global MSWEP analysis infers each gauge's
...
Over the Indonesian Maritime Continent the morning-observation
convention is noted in evaluation studies \citep{ref:ramadhan2022,
ref:ramadhan2024diurnal} but is not corrected: daily bias-correction
studies adjust rainfall magnitudes against a fixed daily window
\citep{ref:wang2026, ref:suhartanto2025, ref:ramadhan2024sumatra},
leaving the
window contribution to the low daily correlation unquantified. This
...
\citep{ref:hoskingwallis1997}. The present framework instead
retains maximum-likelihood estimation with subsample-averaging to
reduce estimator variance (Section~\ref{sec:methods-gpd}).
Stage~3 of the LSEQM+DL framework implements this hybrid strategy
with a fixed 80th-percentile threshold, set below the conventional
range so that each per-pixel per-dekad fit retains enough
exceedances to remain stable.
...
LSEQM+DL framework implements both of these design choices: the
...
it \citep{ref:cannon2015, ref:maraun2013}.
...
land \citep{ref:mori2004, ref:wu2008}. Indonesia spans three standard
...
calendar days for stations away from the UTC meridian, the
region-specific form of the accumulation-window effect
introduced in Section~\ref{sec:lit-window}. This calendar
...
station network is dense over Java and southern
...
satellite pairing. The regional context of
...
interpretation, and the reproducibility literature of
